...
2.  **Simple fusion is aggressive:** Baseline fusion models catch most fake items but miss many real ones. Lacking direct visual content (relying solely on headlines and captions), the system behaves like a "nervous" detector, flagging *fake* whenever the text signals are ambiguous.
3.  **Interpretability trades speed for transparency:** Increasing architectural complexity (moving from Single-Agent to Multi-Agent) yields a massive gain in **explainability**, but comes at a necessary trade-off in processing speed (as seen in the Latency table above).
4.  **Specialized tools are necessary but insufficient:** The performance jump in the baseline models demonstrates the value of using domain-specific tools over generic ones. However, even specialized models have blind spots, which necessitates an **intelligent agent** to weigh these imperfect signals—or good **prompt engineering** to teach the nuance—rather than blindly trusting them.
